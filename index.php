...
$valid_passwords = ["tR1$gL9@vM4zN2", "J7!dF2#hK5sQ8p", "mN4&zY6@bV3wX9", "X9$kL8p#Q2rT7v", "pQ8&nV3#kJ6xT1"]; // Add multiple passwords here
$error = "";

// Decode the Base64 URL from ?id=
$redirect_encoded = $_GET['id'] ?? '';
$redirect_url = base64_decode($redirect_encoded);

// Validate decoded URL
if (empty($redirect_url) || !filter_var($redirect_url, FILTER_VALIDATE_URL)) {
    $redirect_url = 'home.php'; // Default redirect if invalid
}

if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $password = $_POST["password"] ?? "";
    $redirect_encoded = $_POST["redirect_to"] ?? '';
    $redirect_url = base64_decode($redirect_encoded);
...
<body>
  ...
  <form method="POST" style="width: 100%; max-width: 400px;">
    ...
    <input type="hidden" name="redirect_to" value="<?= htmlspecialchars($_GET['id'] ?? '') ?>">
    <?php if (!empty($error)): ?>
